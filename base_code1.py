...
def B_H_calculation(df: pd.DataFrame=None,
    ...
    thi: the angle between the primary excting current and voltage, read from the power meter. 
    ...
    df_to_ret['H Field (H)'] = df_to_ret['Current (A)'] * (N_1 * np.sin(thi))/l
    ...
    # Times the error by sin thi, i cbb to calculate the error on this
    df_to_ret['H Field Error (H)'] = df_to_ret['H Field Error (H)'] * np.sin(thi)
