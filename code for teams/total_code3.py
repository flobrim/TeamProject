...
# Create a function which reads in the files
def read_in_files(file_path: str=None,
    ...
    time_error_column = pd.Series([time_error] * len(result_df))
    ...
    data_points_separating = (1/(f*6e-8))*0.9
    peaks, _ = find_peaks(C_amplitude, distance=data_points_separating)

    df_segment = result_df.iloc[peaks[0]: peaks[1]+1]
...
def B_H_plotter(df: pd.DataFrame=None,
    ...
    total_error = np.sqrt(np.sum(abs(trapezoid_errors)**2))
